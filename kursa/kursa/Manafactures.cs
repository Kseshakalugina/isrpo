...
namespace kursa
{
    public partial class Manafactures : Form
    {
        Add add = new Add();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else if (sender == button4) // изменить 
            {
                ...
                add.change = true;
                add.ShowDialog();
                Update1("SELECT * FROM Производители ORDER BY [Код производителя]");
                textBox1.Text = "";
            }
            else if (sender == button3) // удалить
            {
                ...
                myConnection = new SqlConnection(connectString);
                myConnection.Open();
                string cmdDelFromTovari = "Delete from Производители where [Код производителя] = @code";
                ...
                dataGridView1.Columns.Clear();
                myConnection.Close();
                string sql = "SELECT * FROM Производители ORDER BY [Код производителя]";
                Update1(sql);
                textBox1.Text = "";
            }
            else if (sender == button2) // добавить
            {
                add.change = false;
                add.ShowDialog();
                Update1("SELECT * FROM Производители ORDER BY [Код производителя]");
        ...
        private void Manafactures_Load(object sender, EventArgs e)
        {
            Update1("SELECT * FROM Производители ORDER BY [Код производителя]");
            add.Owner = this;
        }
