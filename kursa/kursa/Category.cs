    ...
    public partial class Category : Form
    {
        ...
        private void Category_Click(object sender, EventArgs e)
        {
            if (sender == button2) // добавить
            {
                add.change = false;
                add.ShowDialog();
                Update1("SELECT * FROM Категории ORDER BY [Код категории]");
            }
            ...
            else if (sender == button3) // удалить
            {
                ...
                myConnection = new SqlConnection(connectString);
                myConnection.Open();
                string cmdDelFromTovari = "Delete from Категории where [Код категории] = @code";
                ...
                dataGridView1.Columns.Clear();
                myConnection.Close();
                string sql = "SELECT * FROM Категории ORDER BY [Код категории]";
                Update1(sql);
                textBox1.Text = "";
            }
